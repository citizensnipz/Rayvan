...
def train_block(model: CounterfactualValueEMC, optimizers: ValueOptimizers, inputs: Tensor, targets: Tensor,
    ...
    states = snapshot.collect_states(inputs, endpoint_targets, generator=optimizers.generator,
                                     exploration=c.value_exploration_rate)
    optimizers.costs["collection_expert_items"] += B * (T - 1)
    pairs = sample_probe_pairs(B, T, c.value_probe_rate, c.value_probe_budget, optimizers.generator)
    probes = []
    ...
    with trainable_only(model, active if c.value_expert_training != "frozen" else []):
        output = model.endpoint(inputs, return_trace=True, exploration=c.value_exploration_rate,
...
@torch.no_grad()
def audit_values(model: CounterfactualValueEMC, inputs: Tensor, targets: Tensor) -> dict:
    """Held-out full-candidate suffix audit, never reused as router training labels."""
    snapshot = frozen_snapshot(model)
    states = snapshot.collect_states(inputs, targets[:, -1], generator=torch.Generator().manual_seed(0), exploration=0)
    rows = []
    for s in states:
        losses = snapshot.counterfactual_losses(s, target="suffix")
        predicted = snapshot.router(s.latent, model.config.resolved_trajectory_steps - s.depth).float()
    ...
    return dict(by_depth=rows, mean_regret=sum(r["regret"] for r in rows) / len(rows),
                ...
                probe_count=inputs.size(0) * len(rows), target="held_out_suffix")
...
def train_value_model(model, corpus, config, *, print_progress=True, evaluation_callback=None,
    ...
    for step in range(start_step + 1, total + 1):
        ...
        with _autocast_context(device, precision):
            block = train_block(model, opts, inputs, targets, step, config.gradient_clip_norm)
        tokens += count
        ...
        values = dict(step=step, tokens_processed=tokens, target_tokens=config.train_tokens,
                      objective="prefix_endpoint", training_loss=block["loss"],
