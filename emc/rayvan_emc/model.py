...
class EMCConfig:
    ...
    def __post_init__(self) -> None:
        ...
        spectral_config(self)
        if self.router_type == "spectral_geometric" and (self.architecture_stage != "n1_sequential" or self.modules_per_cycle != 1):
            raise ValueError("spectral geometry requires sequential top-1 execution")
        positive_fields = {
        ...
        if self.router_type not in {"fixed_index", "module_aware", "geometric", "counterfactual_value", "spectral_geometric"}:
...
class EMCModel(nn.Module):
    def __init__(self, config: EMCConfig) -> None:
        super().__init__()
        if config.router_type == "spectral_geometric" and type(self) in {EMCModel, SequentialEMCModel}:
            raise ValueError("Use SpectralGeometricEMC for prefix-safe spectral execution")
        self.config = config
        ...
        if config.router_type == "spectral_geometric":
            from .spectral_routing import SpectralGeometricRouter
            self.router = SpectralGeometricRouter(config)
        elif config.router_type == "counterfactual_value":
