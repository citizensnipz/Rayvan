...
#[tauri::command]
pub async fn get_token_lab_schema() -> Result<Value, String> {
    run_python_json(&["schema"], None)
}

#[tauri::command]
pub async fn validate_token_lab(config: Value) -> Result<Value, String> {
...
#[tauri::command]
pub async fn start_token_lab(
    app: AppHandle,
    state: State<'_, ExperimentProcessState>,
    request: StartRequest,
) -> Result<StartResponse, String> {
    let mut slot = state.0.lock().map_err(|_| "Token Lab state is unavailable")?;
    if let Some(run) = slot.as_ref() {
        return Err(format!("Token Lab {} is already running", run.run_id));
    }
    ...
    std::thread::spawn(move || {
        for line in BufReader::new(stdout).lines().map_while(Result::ok) {
            match serde_json::from_str::<Value>(&line) {
                Ok(event) => {
                    let _ = stdout_app.emit("token-lab-event", event);
                }
                Err(_) => {
...
#[tauri::command]
pub fn list_token_labs(app: AppHandle) -> Result<Vec<Value>, String> {
...
#[tauri::command]
pub fn get_token_lab(app: AppHandle, run_id: String) -> Result<Value, String> {
...
fn research_runs_directory(app: &AppHandle) -> Result<PathBuf, String> {
    if let Ok(path) = std::env::var("RAYVAN_TOKEN_LAB_DIR") {
        return Ok(PathBuf::from(path));
    }
    app.path()
        .app_local_data_dir()
        .map(|path| path.join("token-lab-runs"))
        .map_err(|error| error.to_string())
}
