  ...
  return <section className="history-view">
    ...
    <div className="table-wrap"><table id="history-table"><thead><tr><th /><th>Date / run</th><th>Suite</th><th>Architecture</th><th>Targets</th><th>Val loss</th><th>Perplexity</th><th>Target throughput</th><th>Context tok/s</th><th>Runtime</th><th>State</th><th>Commit</th></tr></thead><tbody>{runs.map((run) => {
      ...
      return <tr key={run.run_id} data-suite={run.suite} data-architecture={run.architecture} data-status={run.status} data-tokens={typeof headline.tokens_processed === "number" ? headline.tokens_processed : 0} data-date={run.started_at ? new Date(run.started_at).getTime() : 0} data-search={`${run.name} ${(run.tags ?? []).join(" ")}`.toLowerCase()} onDoubleClick={() => onOpen(run.run_id)}>
        ...
        <td>{run.suite}</td><td>{run.architecture.replaceAll("_", " ")}</td><td>{typeof headline.tokens_processed === "number" ? headline.tokens_processed.toLocaleString() : "—"} {fixedFit ? "router updates" : endpoint ? "endpoints" : "tokens"}</td><td>{metric(headline.validation_loss)}</td><td>{metric(headline.perplexity, 2)}</td><td>{metric(headline.tokens_per_second, 2)} {fixedFit ? "updates/s" : endpoint ? "endpoint/s" : "tok/s"}</td><td>{metric(headline.context_tokens_per_second ?? (!endpoint ? headline.tokens_per_second : undefined), 0)}</td><td>{formatDuration(headline.runtime_seconds)}</td><td><span className={`status ${run.status}`}>{run.status}</span></td><td className="mono">{shortHash(run.git?.commit)}{run.git?.dirty ? "*" : ""}</td>
      </tr>;
    })}</tbody></table></div>
