...
export function RunComparison({ runs }: { runs: RunDetail[] }) {
  ...
  const mixedObjectives = runs.some(endpoint) && runs.some((run) => !endpoint(run));
  ...
  return <section className="compare-view">
    ...
    <p>Context tok/s counts main input exposures, excludes repeated expert/probe work, and is not generation throughput. {mixedObjectives ? "These runs use different objectives; endpoint counts and all-position token counts are not equivalent training budgets." : "Compare routing regret and validation quality alongside elapsed time."}</p>
    <div className="comparison-table panel"><table><thead><tr><th>Run</th><th>Architecture</th><th>Targets</th><th>Val loss</th><th>Perplexity</th><th>Target throughput</th><th>Context tok/s</th><th>Total params</th><th>Active params</th><th>FLOPs/token</th><th>Peak VRAM</th></tr></thead><tbody>{runs.map((run) => <tr key={run.runId}><td><b>{run.summary?.name ?? run.runId}</b></td><td>{run.config?.architecture}</td><td>{fmt(run.summary?.headline?.tokens_processed, 0)} {endpoint(run) ? "endpoints" : "tokens"}</td><td>{fmt(run.summary?.headline?.validation_loss)}</td><td>{fmt(run.summary?.headline?.perplexity, 2)}</td><td>{fmt(run.summary?.headline?.tokens_per_second, 2)} {endpoint(run) ? "endpoint/s" : "tok/s"}</td><td>{fmt(contextRate(run, run.summary?.headline), 0)}</td><td>{fmt(run.model?.total_parameters, 0)}</td><td>{fmt(run.model?.approximate_active_parameters, 0)}</td><td>{fmt(run.model?.approximate_flops_per_token, 0)}</td><td>{bytes(run.summary?.headline?.peak_vram_bytes)}</td></tr>)}</tbody></table></div>
