...
export function ExperimentBuilder({ schema, config, setConfig, estimate, estimating, active, onRun }: {
  ...
  const usesExperts = ["emc", "heterogeneous_serial", "old_emc"].includes(config.architecture);
  ...
  return <div className="builder-layout">
    <section className="builder-form">
      ...
      <label><span>Architecture</span><select value={config.architecture} onChange={(event) => setRoot("architecture", event.target.value)}>{schema.architectures.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</select></label>
      ...
      <div className="field-grid three">
        {!['homogeneous_serial', 'heterogeneous_serial'].includes(config.architecture) && <NumberField label="Top-K" value={Number(config.routing.top_k)} min={1} max={Math.max(expertCount, 1)} onChange={(value) => setNested("routing", "top_k", value)} />}
        ...
        {config.architecture === "old_emc" && <label><span>Router</span><select value={String(config.routing.router_type)} onChange={(event) => setNested("routing", "router_type", event.target.value)}><option value="module_aware">Module-aware Nexus</option><option value="fixed_index">Fixed-index baseline</option></select></label>}
        {config.architecture === "old_emc" && <label><span>Integrator</span><select value={String(config.routing.integrator_type)} onChange={(event) => setNested("routing", "integrator_type", event.target.value)}><option value="proposal_attention">Proposal attention</option><option value="weighted_average">Weighted average</option></select></label>}
        {["emc", "old_emc"].includes(config.architecture) && <NumberField label="Balance loss weight" value={Number(config.routing.balance_coefficient)} step={0.001} min={0} onChange={(value) => setNested("routing", "balance_coefficient", value)} />}
        {["emc", "old_emc"].includes(config.architecture) && <NumberField label="Entropy floor" value={Number(config.routing.balance_entropy_floor)} step={0.05} min={0} max={1} onChange={(value) => setNested("routing", "balance_entropy_floor", value)} />}
        {config.architecture === "emc" && <>
          ...
          <NumberField label="Minimum lease chunks" value={Number(config.routing.minimum_lease_chunks)} min={0} onChange={(value) => setNested("routing", "minimum_lease_chunks", value)} />
      ...
      <div className="field-grid three">
        ...
        <NumberField label="Chunk size" value={Number(config.model.chunk_size)} min={1} onChange={(value) => setNested("model", "chunk_size", value)} />
