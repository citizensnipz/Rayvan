  ...
  const changeArchitecture = (architecture: string) => {
    ...
    if (architecture === "counterfactual_value_emc") {
      ...
      Object.assign(routing, {
        ...
        value_fixed_reference: true, value_reset_router: true, value_checkpoint_path: "", value_fit_enabled: false, value_head_type: "linear", value_fit_bank_path: "",
      });
    }
    ...
    <section className="builder-form">
      ...
      <div className="field-grid three">
        ...
        {valueRouting && <>
          ...
          <label><span>Prediction head</span><select value={String(config.routing.value_head_type ?? "linear")} onChange={(e) => setConfig({ ...config, routing: { ...config.routing, value_head_type: e.target.value, ...(["geometric", "expert_geometric"].includes(e.target.value) ? { routing_geometry_dim: 32 } : {}), ...(e.target.value === "expert_geometric" ? { value_expert_training: "frozen", value_fixed_reference: true, value_reset_router: true, value_target: "suffix", value_fit_enabled: false, value_fit_bank_path: "", value_effect_dim: 16, value_effect_weight: 0.01, value_replay_capacity: 1024, value_replay_batch_size: 64 } : {}) } })}><option value="linear">Linear</option><option value="mlp">Nonlinear (GELU)</option><option value="geometric">Relational geometric (slots + basins)</option><option value="expert_geometric">Expert-conditioned geometric + effects + replay</option></select></label>
