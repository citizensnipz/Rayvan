...
export function TokenLab({initialConfig}:{initialConfig?:Obj}={}){
 ...
 const [config,setConfig]=useState<Obj|undefined>(initialConfig),[runs,setRuns]=useState<Obj[]>([]),[detail,setDetail]=useState<Obj>(),[progress,setProgress]=useState<Obj>(),[routingEvents,setRoutingEvents]=useState<Obj[]>([]),[active,setActive]=useState<string>(),[error,setError]=useState(''),[notice,setNotice]=useState(''),[mixed,setMixed]=useState(false),[tasks,setTasks]=useState<string[]>([]);
 ...
 useEffect(()=>{let gone=false;const off: Array<()=>void>=[];
  ...
  for(const [name,handler] of [['token-lab-log',(e:Obj)=>setNotice(String(e.line??''))],['token-lab-event',(e:Obj)=>{setProgress(e);if(e.accepted!==undefined&&!e.heartbeat)setRoutingEvents(v=>[...v,e].slice(-100));}],['token-lab-process-exit',(e:Obj)=>{setActive(undefined);void reopen(e.runId).catch(e=>setError(String(e)));void refresh();}]] as const){void listen<Obj>(name,e=>{if(!gone)handler(e.payload);}).then(f=>{if(gone)f();else off.push(f);});}
 ...
 const action=async(mode:string)=>{try{setError('');if(mode==='validate'){const r=await invoke<Obj>('validate_token_lab',{config});setNotice(r.warning);}
  if(mode==='run'){await invoke('validate_token_lab',{config});const r=await invoke<Obj>('start_token_lab',{request:{config}});setActive(r.runId);setDetail(undefined);setProgress(undefined);setRoutingEvents([]);}
  if(mode==='stop')await invoke('cancel_token_lab');}catch(e){setError(String(e));}};
 const discovery=async(request:Obj)=>{try{setError('');await invoke('validate_token_lab',{config:request});const r=await invoke<Obj>('start_token_lab',{request:{config:request}});setActive(r.runId);setDetail(undefined);setProgress(undefined);setRoutingEvents([]);}catch(e){setError(String(e));}};
 ...
 return <div className="token-lab">
  <section className="panel"><h2>Token Lab · research workspace</h2><p>{routing?'Frozen Expert Routing Test separately tests learned choices using saved experts. All experts and shared components stay frozen; no production EMC changes.':'Standalone experts with a common contextual encoder and readout. No EMC router or performance-based training allocation. Calibration deliberately controls task exposure; standard mode does not.'} Avoid running another GPU experiment concurrently.</p>
   <fieldset disabled={Boolean(active)}><div className="lab-fields">
    <label>Experiment mode<select value={config.experiment_mode??'standard'} onChange={e=>changeMode(e.target.value)}><option value="standard">Standard Token Lab</option><option value="forced_specialization">Forced Specialization Calibration</option><option value="validation_sweep">Specialization Validation Sweep</option><option value="frozen_routing">Frozen Expert Routing Test</option></select></label>
    {routing?<RoutingSetup config={config} setConfig={setConfig} runs={runs}/>:sweep?<SweepSetup config={config} setConfig={setConfig}/>:<>
  ...
  {(sweep||routing)&&<p>Experiment progress: {progress?.completed_jobs??detail?.summary?.completed_jobs??0} / {progress?.planned_jobs??detail?.summary?.planned_jobs??(sweep?8*config.sweep_seed_count:'—')} jobs. {progress?.phase}</p>}
  ...
  {!routing&&!sweep&&detail?.summary?.status==='completed'&&!detail?.analysis?.sweep&&!detail?.analysis?.routing_test&&<DiscoverySetup key={'discovery-'+detail.runId} detail={detail} active={Boolean(active)} onRun={r=>void discovery(r)}/>}
  ...
  {detail?.analysis&&!detail.analysis.discovery&&!detail.analysis.sweep&&!detail.analysis.routing_test&&(detail.analysis.calibration?<details><summary>Secondary standard explorer and full report · includes outcomes and task labels</summary><Explorer key={detail.runId} detail={detail}/></details>:<Explorer key={detail.runId} detail={detail}/>)}
 </div>;
}
