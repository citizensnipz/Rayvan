...
import {RoutingSetup,RoutingResults,RoutingProgress,routingDefaults} from './FrozenRouting';
import {EmergenceSetup,EmergenceResults,EmergenceProgress,emergenceDefaults} from './EmergentSpecialization';
...
export function TokenLab({initialConfig}:{initialConfig?:Obj}={}){
 ...
 const [config,setConfig]=useState<Obj|undefined>(initialConfig),[runs,setRuns]=useState<Obj[]>([]),[detail,setDetail]=useState<Obj>(),[progress,setProgress]=useState<Obj>(),[routingEvents,setRoutingEvents]=useState<Obj[]>([]),[emergenceEvents,setEmergenceEvents]=useState<Obj[]>([]),[active,setActive]=useState<string>(),[error,setError]=useState(''),[notice,setNotice]=useState(''),[mixed,setMixed]=useState(false),[tasks,setTasks]=useState<string[]>([]);
 const refresh=()=>invoke<Obj[]>('list_token_labs').then(setRuns);
 const reopen=async(id:string)=>{const d=await invoke<Obj>('get_token_lab',{runId:id});setDetail(d);setProgress(undefined);setEmergenceEvents((d.events??[]).filter((e:Obj)=>e.interaction_rms!==undefined).slice(-200));};
 useEffect(()=>{let gone=false;const off: Array<()=>void>=[];
  ...
  for(const [name,handler] of [['token-lab-log',(e:Obj)=>setNotice(String(e.line??''))],['token-lab-event',(e:Obj)=>{setProgress(e);if(e.interaction_rms!==undefined&&!e.heartbeat)setEmergenceEvents(v=>[...v,e].slice(-200));if(e.accepted!==undefined&&!e.heartbeat)setRoutingEvents(v=>[...v,e].slice(-100));}],['token-lab-process-exit',(e:Obj)=>{setActive(undefined);void reopen(e.runId).catch(e=>setError(String(e)));void refresh();}]] as const){void listen<Obj>(name,e=>{if(!gone)handler(e.payload);}).then(f=>{if(gone)f();else off.push(f);});}
  return()=>{gone=true;off.forEach(f=>f());};
 },[]);
 const set=(key:string,value:any)=>setConfig(c=>({...c,[key]:value}));
 const changeMode=(mode:string)=>{
  setMixed(false);
  if(mode==='emergent_specialization'){setConfig(c=>({...emergenceDefaults,device:c?.device??'cpu',source_sweep:detail?.analysis?.sweep?detail.runDirectory:''}));return;}
  if(mode==='frozen_routing'){setConfig(c=>({...routingDefaults,device:c?.device??'cpu',source_sweep:detail?.analysis?.sweep?detail.runDirectory:''}));return;}
 ...
 const action=async(mode:string)=>{try{setError('');if(mode==='validate'){const r=await invoke<Obj>('validate_token_lab',{config});setNotice(r.warning);}
  if(mode==='run'){await invoke('validate_token_lab',{config});const r=await invoke<Obj>('start_token_lab',{request:{config}});setActive(r.runId);setDetail(undefined);setProgress(undefined);setRoutingEvents([]);setEmergenceEvents([]);}
  if(mode==='stop')await invoke('cancel_token_lab');}catch(e){setError(String(e));}};
 const discovery=async(request:Obj)=>{try{setError('');await invoke('validate_token_lab',{config:request});const r=await invoke<Obj>('start_token_lab',{request:{config:request}});setActive(r.runId);setDetail(undefined);setProgress(undefined);setRoutingEvents([]);setEmergenceEvents([]);}catch(e){setError(String(e));}};
 ...
 const emergence=config.experiment_mode==='emergent_specialization';
 const routing=config.experiment_mode==='frozen_routing';
 ...
 return <div className="token-lab">
  <section className="panel"><h2>Token Lab · research workspace</h2><p>{emergence?'Controlled expert development with task-agnostic asymmetry and competence feedback. No production EMC changes.':routing?'Frozen Expert Routing Test separately tests learned choices using saved experts. All experts and shared components stay frozen; no production EMC changes.':'Standalone experts with a common contextual encoder and readout. No EMC router or performance-based training allocation. Calibration deliberately controls task exposure; standard mode does not.'} Avoid running another GPU experiment concurrently.</p>
   <fieldset disabled={Boolean(active)}><div className="lab-fields">
    <label>Experiment mode<select value={config.experiment_mode??'standard'} onChange={e=>changeMode(e.target.value)}><option value="standard">Standard Token Lab</option><option value="forced_specialization">Forced Specialization Calibration</option><option value="validation_sweep">Specialization Validation Sweep</option><option value="frozen_routing">Frozen Expert Routing Test</option><option value="emergent_specialization">Emergent Specialization</option></select></label>
    {emergence?<EmergenceSetup config={config} setConfig={setConfig} runs={runs}/>:routing?<RoutingSetup config={config} setConfig={setConfig} runs={runs}/>:sweep?<SweepSetup config={config} setConfig={setConfig}/>:<>
  ...
  {(emergence||detail?.analysis?.emergence)?<section className="panel"><h3>Run status</h3><p>{active??detail?.runId??'No run selected'} · {progress?.phase??detail?.summary?.status??'Idle'}</p><p>{progress?.completed_jobs??detail?.summary?.completed_jobs??0} / {progress?.planned_jobs??detail?.summary?.planned_jobs??'—'} jobs · {fmt(progress?.elapsed_seconds??detail?.summary?.runtime_seconds)} seconds elapsed · estimated remaining {fmt(progress?.estimated_remaining_seconds)} seconds.</p><p>Current job training applications: {progress?.training_targets??'—'}; training context-token exposures: {progress?.training_context_tokens??'—'}. These are expert workload counts with cached common states, not generated tok/s. Per-checkpoint costs are saved separately.</p>{detail?.status?.error&&<p role="alert">{detail.status.error}</p>}{detail?.logs&&<details><summary>Logs</summary><pre>{detail.logs}</pre></details>}</section>:<section className="panel"><h3>Run status</h3><p>{active??detail?.runId??'No run selected'} · {progress?.phase??detail?.summary?.status??detail?.status?.status??'Idle'}</p><p>{progress?.step??0} / {progress?.total??'—'} · {progress?.measured_locations??detail?.summary?.measured_locations??0} locations · {progress?.observations??detail?.summary?.observations??0} expert observations · {fmt(progress?.elapsed_seconds??detail?.summary?.runtime_seconds)} seconds</p><p>Current expert/stage: {progress?.expert_id??'—'} / {progress?.expert_stage??'—'}. Training endpoint targets: {progress?.training_targets??detail?.summary?.training_targets??'—'}. Training context exposures: {progress?.training_context_tokens??detail?.summary?.training_context_tokens??'—'} tokens. Exact-prefix exclusions: {detail?.summary?.duplicate_prefixes_skipped??'—'}.</p><p>End-to-end measured locations/s: {fmt(detail?.summary?.locations_per_second)}. Probe + measurement seconds: {fmt(detail?.summary?.probe_and_measurement_seconds)} (includes expert execution; not isolated overhead).</p>{detail?.status?.error&&<p role="alert">{detail.status.error}</p>}<p>Feature/diagnostic time excluding instrumented expert forward: {fmt(detail?.summary?.feature_and_diagnostic_seconds)} seconds. FFN hook overhead remains counted with expert execution.</p>{detail?.logs&&<details><summary>Logs</summary><pre>{detail.logs}</pre></details>}</section>}
  {(sweep||routing||emergence)&&<p>Experiment progress: {progress?.completed_jobs??detail?.summary?.completed_jobs??0} / {progress?.planned_jobs??detail?.summary?.planned_jobs??(sweep?8*config.sweep_seed_count:'—')} jobs. {progress?.phase}</p>}
  {detail?.analysis?.sweep&&<SweepResults key={detail.runId} detail={detail} onRouting={()=>{if(!active)setConfig({...routingDefaults,source_sweep:detail.runDirectory,device:config.device??'cpu'});}} onResume={c=>{if(!active)setConfig(c);}}/>}
  {emergence&&active&&<EmergenceProgress events={emergenceEvents} progress={progress}/>}
  {detail?.analysis?.emergence&&<EmergenceResults key={detail.runId} detail={detail}/>}
  {routing&&active&&<RoutingProgress events={routingEvents} progress={progress}/>}
  {detail?.analysis?.routing_test&&<RoutingResults key={detail.runId} detail={detail}/>}
  {!emergence&&!routing&&!sweep&&detail?.summary?.status==='completed'&&!detail?.analysis?.sweep&&!detail?.analysis?.routing_test&&!detail?.analysis?.emergence&&<DiscoverySetup key={'discovery-'+detail.runId} detail={detail} active={Boolean(active)} onRun={r=>void discovery(r)}/>}
  ...
  {detail?.analysis&&!detail.analysis.discovery&&!detail.analysis.sweep&&!detail.analysis.routing_test&&!detail.analysis.emergence&&(detail.analysis.calibration?<details><summary>Secondary standard explorer and full report · includes outcomes and task labels</summary><Explorer key={detail.runId} detail={detail}/></details>:<Explorer key={detail.runId} detail={detail}/>)}
 </div>;
}
