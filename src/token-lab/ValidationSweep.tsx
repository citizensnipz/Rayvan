...
export const sweepDefaults={experiment_mode:'validation_sweep',name:'Specialization Validation Sweep',
 sweep_seed_start:42,sweep_seed_count:3,diagnostic_seed:42,common_pretrain_steps:1000,train_steps:1000,
...
export function SweepSetup({config,setConfig}:{config:Obj;setConfig:(v:any)=>void}){
 ...
 return <section className="panel" style={{gridColumn:'1 / -1'}}><h3>Specialization Validation Sweep</h3>
  <p>GPT, SSM, recurrent/GRU and Delta modules. Two same-family experts each time; 0% and 100%; no router. These are whole expert modules, not interchangeable FFN activations.</p>
  <p><strong>{8*config.sweep_seed_count} calibration runs</strong> plus automatic discovery and comparison. One common base per family/seed, reused exactly for its control. Jobs run sequentially on your selected device.</p>
  ...
  <p>Automatic: balanced 10-task common pretraining; calculation vs memory/sequence profile; matched budgets; identical-stream 0% controls; independent held-out probes; target-free feature discovery; feature-group comparisons; shuffled-label sanity check; offline choice regret; combined report. No checkpoint paths to choose.</p>
...
export function SweepResults({detail,onResume}:{detail:Obj;onResume:(config:Obj)=>void}){
 ...
 return <section className="panel"><h2>Specialization Validation Sweep results</h2>
  ...
  <p>Positive gain means a diagnostic predictor’s choices beat the training-selected constant expert on held-out observations. Zero/negative results stay visible. Cross-family comparisons also change the common representation and parameter counts.</p>
  ...
  {e&&<><p>{e.error}</p><p>Source: {e.source_directory??'not completed'}</p>
   <p>Cloned base: {e.controls?.common_hash??'unverified'} · shared frozen: {String(e.controls?.shared_unchanged??'unverified')} · exact final identity: {String(e.controls?.exact_control_final_identity??'unverified')}</p>
   ...
   <table><thead><tr><th>Input set</th><th>Test R²</th><th>MAE</th><th>MAE minus all</th><th>95% interval</th><th>Choice gain</th></tr></thead><tbody>{Object.entries(e.extra?.groups??{}).map(([k,g]:any)=><tr key={k}><td>{k}</td><td>{fmt(g.scores.r2)}</td><td>{fmt(g.scores.mae)}</td><td>{fmt(g.mae_minus_all)}</td><td>{g.mae_minus_all_ci?.map(fmt).join(' … ')??'—'}</td><td>{fmt(g.decisions.gain_over_constant)}</td></tr>)}</tbody></table>
   <p>Shuffled-label test R²: {fmt(e.extra?.shuffled_label_check?.scores?.r2)}. A single negative sanity check does not establish statistical significance.</p>
   <h3>Transfer to excluded tasks</h3><table><thead><tr><th>Excluded task</th><th>Test n</th><th>All-input R²</th><th>Controls R²</th><th>All-input choice gain</th></tr></thead><tbody>{(e.extra?.cross_task??[]).map((t:Obj)=><tr key={t.task}><td>{t.task} {t.status!=='ok'?t.status:''}</td><td>{t.test_n??'—'}</td><td>{fmt(t.all?.scores?.r2)}</td><td>{fmt(t.controls?.scores?.r2)}</td><td>{fmt(t.all?.decisions?.gain_over_constant)}</td></tr>)}</tbody></table>
